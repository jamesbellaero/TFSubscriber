...
Code related to I/O sampling and configuration (the A\+T\+IS command, 0x92 frames).  
...
\begin{DoxyNote}{Note}
This code is not directly concerned with collecting I/O samples. To do that, it is necessary to register a command handler for A\+T\+IS commands, and/or appropriate endpoints to handle the incoming data frames. If not using the Simple X\+Bee A\+PI (S\+XA) layer, then see \hyperlink{xbee__sxa_8c}{xbee\+\_\+sxa.\+c} for examples of how to register the necessary callback functions. Otherwise, it is recommended to use S\+XA for all I/O manipulation. 
\end{DoxyNote}
